...
(function($)
{
	...
	{
		string = $editor.display.lineDiv.innerHTML;
		...
		// remove first line number
		regex = new RegExp('<div class="CodeMirror-linenumber[^>]*>1</div>', 'gim');
		string = string.replace(regex, '');
		// replace rest of line numbers with a break
		regex = new RegExp('<div class="CodeMirror-linenumber[^>]*>.*?</div>', 'gim');
		string = string.replace(regex, '<br />');
		// remove aal div/span tags
		regex = new RegExp('</?(span|div)( [^>]*)?>', 'gim');
		string = string.replace(regex, '');
